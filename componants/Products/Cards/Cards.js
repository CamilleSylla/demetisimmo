...
export default function Cards({ data }) {
  ...
  const Disponibilite = () => {
    return (
      <div
        className={style.disponibility}
        style={data.acf.dispo == "Vrai" ? disponible : vendu}
      >
        <p style={{ color: "white" }}>
          {data.acf.dispo == "Vrai" ? "Disponible" : "Vendu"}
        </p>
      </div>
    );
  };
  ...
  const Agent = () => {
    return <img src={agent} className={style.agent} />;
  };
    ...
    <Link href={`/bien/${data.id}`}>
      <article className={style.wrapper}>
        ...
        <span className={style.price}>
          {numberWithSpaces(data.acf.prix.toLocaleString())}€
        </span>
        <h1>
          {load === true ? decodeHTMLEntities(data.title.rendered) : null}
        </h1>
          ...
          {data.acf.type === "Terrain" ? null : (
            <>
              ...
              <div className={style.icon_wrapper}>
                ...
                <p>
                  {data.acf.chambre} chr{data.chambres > 1 ? "s" : null}
                </p>
              </div>
            </>
          )}
